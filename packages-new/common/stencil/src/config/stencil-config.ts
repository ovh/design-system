...
interface OdsStencilConfigEnvOptions {
  tsConfig?: StencilConfig['tsconfig'];
  globalScript?: StencilConfig['globalScript'];
}
...
export function getStencilConfig({
  ...
  if (distCustomElements && Array.isArray(distCustomElements.copy)) {
    ...
    if (isProd) {
      // TODO Figure out why this is necessary and try to remove it!
