import type { OdsFileI } from '../interfaces/attributes';
import { OdsFileController } from './controller';
import { OsdsFile } from '../osds-file';

class OdsFileMock extends OsdsFile {
  constructor(attribute: Partial<OsdsFile>) {
    super();
...
describe('OdsFileController', () => {
  ...
  describe('event', () => {
    it('should emit files with accepted types from dataTransfer from dataTransfer.files', () => {
      ...
      component = new OdsFileMock({ acceptedTypes: 'image/png' });
      controller = new OdsFileController(component);
      controller.onDrop(event);

      expect(event.preventDefault).toHaveBeenCalled();
      expect(component.dragOver).toBe(false);
      expect(component.emitFiles).toHaveBeenCalledTimes(1);
      expect(component.emitRejectedFile).toHaveBeenCalledTimes(1);
    });
