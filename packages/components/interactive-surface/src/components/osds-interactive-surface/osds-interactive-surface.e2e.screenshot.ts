...
describe('e2e:osds-interactive-surface', () => {
  ...
  async function setup({ attributes = {}, html = '' }: { attributes?: Partial<OdsInteractiveSurfaceAttribute>, html?: string } = {}) {
    ...
    await page.setContent(`
      <osds-interactive-surface ${odsStringAttributes2Str(stringAttributes)}>
        ${html}
      ...
      slotConfigurations.forEach(( slotConfiguration ) => {
        attributeConfigurations.forEach(( attributes ) => {
          ...
          it(`${slotConfiguration.description} with ${screenshotConfiguration.description} and ${attributeDescription}`, async() => {
            ...
            await page.evaluate(() => {
              const element = document.querySelector('osds-interactive-surface') as HTMLElement;
              return { width: element.clientWidth, height: element.clientHeight };
            });

            await page.setViewport({ width: 600, height:600 });

            const results = await page.compareScreenshot('interactive-surface', { fullPage: false, omitBackground: true });
