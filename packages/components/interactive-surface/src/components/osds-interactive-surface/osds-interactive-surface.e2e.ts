...
describe('e2e:osds-interactive-surface', () => {
  let page: E2EPage;
  let el: E2EElement;

  async function setup({ attributes, content }: { attributes: Partial<OdsInteractiveSurfaceAttribute>, content: string }) {
    ...
    await page.setContent(`<osds-interactive-surface ${odsStringAttributes2Str(stringAttributes)}>${content}</osds-interactive-surface>`);
    await page.evaluate(() => document.body.style.setProperty('margin', '0px'));

    el = await page.find('osds-interactive-surface');
  }
  ...
  it('should render with an input', async() => {
    ...
    `;
    await setup({ attributes: {}, content });
    const input = await page.find('osds-interactive-surface osds-input');
  ...
  it('should render with textarea', async() => {
    ...
    `;
    await setup({ attributes: {}, content });
    const textarea = await page.find('osds-interactive-surface osds-textarea');
  ...
  it('should render with tooltip', async() => {
    ...
    `;
    await setup({ attributes: {}, content });
    const tooltip = await page.find('osds-interactive-surface osds-tooltip[slot="visual-hint"]');
  ...
  it('should render inner text content', async() => {
    ...
    `;
    await setup({ attributes: {}, content });
    const label = await page.find('osds-interactive-surface osds-text[slot="label"]');
    const visualHint = await page.find('osds-interactive-surface osds-text[slot="visual-hint"]');
    const input = await page.find('osds-interactive-surface osds-input');
    const helper = await page.find('osds-interactive-surface osds-text[slot="helper"]');
  ...
  it('should switch helper text according to error', async() => {
    ...
    const error = await page.find('osds-interactive-surface >>> osds-text[color="error"]');
