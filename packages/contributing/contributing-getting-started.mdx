...
`ODS` is based on `open source` concept and all contributions are very appreciates.
...
ⓘ When you change files inside the storybook folders `packages/tools/storybook` (`tsx` and `mdx` stories), the server
will live reload and updates the storybook

### Assisted live updates
...
You can manually watch a component and see its changes in Storybook every time you change something in your code.
Go to the folder of the component you want to watch and build it:
...
And you can start Storybook (from Storybook folder):

```shell
cd packages/tools/storybook
yarn start
```

ⓘ All changes in storybook stories and your component you are building will trigger the live update.
...
- `global.dev.ts`: main typescript code for the dev server (not compiled in `build:prod`)

> ⓘ all components are developed with `StencilJs` today and are located into `packages/stencil/components/`
