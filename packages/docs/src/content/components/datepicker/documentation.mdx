...
<IdentityCard aliases={ ['Calendar'] }
              extraLinks={ [<DocLink to="/guides/form-guidelines">Form Guidelines</DocLink>] }
              figmaLink="https://www.figma.com/design/9jDDTcR4a9jPRFcdjawAlf/ODS---UI-Kit?node-id=26-7958"
              githubUrl="https://github.com/ovh/design-system/tree/master/packages/ods-react/src/components/datepicker"
              name="Datepicker">
...
<Anatomy src="components/datepicker/anatomy.png" />
...
Navigating to previous/next month : By clicking on arrow icon <DocLink to="/components/button">Buttons</DocLink> to navigate through months

Selecting a month : By clicking on the month selection <DocLink to="/components/button">Button</DocLink> to switch to month picking mode

Picking a date : By clicking on a date

**Month picking mode**

Navigating to previous/next year : By clicking on arrow icon <DocLink to="/components/button">Buttons</DocLink> to navigate through years

Selecting a year : By clicking on the month selection <DocLink to="/components/button">Button</DocLink> to switch to year picking mode

Picking a month: By clicking on a month, the dropdown goes back to date picking mode

**Year picking mode**

Navigating to previous/next range of years : By clicking on arrow icon <DocLink to="/components/button">Buttons</DocLink> to navigate through years

Selecting a decade : By clicking on the month selection <DocLink to="/components/button">Button</DocLink> to switch to year picking mode
...
Pressing <Kbd>Escape</Kbd> closes the Datepicker dropdown without selecting a date.

Pressing <Kbd>Tab</Kbd> or <Kbd>Shift</Kbd> + <Kbd>Tab</Kbd> moves focus through dropdown controls when open.

<Heading label="Navigating days" level={ 3 } />

- <Kbd>Arrow Left</Kbd>: Move to the previous day
- <Kbd>Arrow Right</Kbd>: Move to the next day
- <Kbd>Arrow Up</Kbd>: Move to the same weekday of the previous week
- <Kbd>Arrow Down</Kbd>: Move to the same weekday of the next week

<Heading label="Jumping within the calendar" level={ 3 } />

- <Kbd>Home</Kbd>: Move to the first day of the current month
- <Kbd>End</Kbd>: Move to the last day of the current month
- <Kbd>Page Up</Kbd>: Move to the same date of the previous month
- <Kbd>Page Down</Kbd>: Move to the same date of the next month

<Heading label="Selecting a date or changing view" level={ 3 } />

- <Kbd>Enter</Kbd>: Select the currently focused date and close the dropdown
- Typing a valid date format (e.g., yyyy-mm-dd) in the input and pressing <Kbd>Enter</Kbd> will also select the date

<Heading label="Navigating to month/year view" level={ 3 } />

- <Kbd>Ctrl</Kbd> / <Kbd>Cmd</Kbd> + <Kbd>Arrow Up</Kbd> (first press): Switches to month selection view
- <Kbd>Ctrl</Kbd> / <Kbd>Cmd</Kbd> + <Kbd>Arrow Up</Kbd> (second press): Switches to year selection view
- Use <Kbd>Arrow</Kbd> keys to move within month/year grids
- Press <Kbd>Enter</Kbd> to confirm selection and return to the previous view
- <Kbd>Ctrl</Kbd> / <Kbd>Cmd</Kbd> + <Kbd>Arrow Left</Kbd> / <Kbd>Arrow Right</Kbd>: Quickly navigate to the previous or next month from date view
...
<Canvas story="AccessibilityDateFormat" />
