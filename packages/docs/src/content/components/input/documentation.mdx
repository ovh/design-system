...
4. **Start content** - optional decoration before the value
5. **End content** - optional decoration after the value
...
Decorative content can be placed inside the field, before the value with `startContent` and after it with `endContent` — a unit such as `kg` or `%`, a currency symbol, a domain extension, an icon, or a small ghost button. It is purely visual or interactive decoration: it is never part of the value and is never submitted with the form.
...
Decoration that carries meaning — a unit, a currency, a fixed part of the value — is therefore silent unless you reference it. Give it an id and point the field at it:
