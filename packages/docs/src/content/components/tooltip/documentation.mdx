...
- Both components look similar but a **Tooltip** is displayed on hover and focus while a <DocLink to="/components/popover">Popover</DocLink> is triggered by click.
...
<Anatomy src="components/tooltip/anatomy.png" />
...
Pressing <Kbd>Tab</Kbd> moves focus to the trigger, opening the **Tooltip** without delay.

Pressing <Kbd>Tab</Kbd> again moves focus away, closing the **Tooltip**.

Pressing <Kbd>Escape</Kbd> closes the **Tooltip** immediately if it is open.
...
<Canvas story="AccessibilityTooltip" />
