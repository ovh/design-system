...
For now, only the positions that correspond to a `vertical` positioning are defined (`green tick`).
Some are not available because of the overlapping of the `surface` container on top of the `anchor` (`red cross`).
The other will be defined in a future version for positions that can be useful for `horizontal` direction (`soon` tag).
...
![Symmetry position table](cdk-images/cdk-overlay-symmetry-position-table.jpg)
