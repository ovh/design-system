...
const ComboboxControl: FC<ComboboxControlProp> = forwardRef(({
  ...
  const { disabled, getContentProps, multiple, open, setOpen, setValue, value } = useComboboxContext();
  const { i18n, invalid, items, locale, readOnly, setItems, vendorItems } = useCombobox();
  ...
  function handleTagClick(tagValue: string): void {
    if (isInteractive) {
      ...
      // setItems(vendorItems.filter((item) => newValue.indexOf(item.value) < 0));
      // // reset();
      // removeTag(tagValue);
      ...
      // TODO update idx given how many values are selected (but some are created values!!)
      //const idx = items.findIndex((item) => item.value === tagValue);
      // const idx = vendorItems.findIndex((item) => item.value === tagValue);
      // //
      // if (idx >= 0) {
      //   insert(idx, vendorItems[idx]);
      // // //   insert(idx, items[idx]);
      // }
    }
  }
...
// console.log(`removing tag value ${tagValue} from ${value}`)
    const newValue = removeValueFromArray(value, tagValue);
    setItems(vendorItems.filter((item) => newValue.indexOf(item.value) < 0));
    setValue(newValue);
    // clearValue(tagValue);
