...
describe('ods-quantity react', () => {
  ...
  it('trigger the odsChange handler on type', async () => {
    const elem = await page.$('ods-quantity:not([is-disabled]) >>> input');
    let consoleLog = '';
    ...
    expect(consoleLog).toBe('React quantity odsChange');
  });

  it('does not trigger the odsChange handler on type if disabled', async () => {
    const elem = await page.$('ods-quantity[is-disabled] >>> input');
    let consoleLog = '';
    page.on('console', (consoleObj) => {
      consoleLog = consoleObj.text();
    });
