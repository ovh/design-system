...
import { isElementInContainer, isTargetInElement } from '../../../../../utils/dom';
...
function debounce(callback: () => unknown): void {
  clearTimeout(bounce);
...
export class OdsCombobox {
  ...
  // FIXME This is triggered on too many use cases (like item click), it is har to rely on it correctly
  private onBlur(): void {
    this.searchElement!.classList.remove('ods-combobox__search--focused');
    ...
    this.isInvalid = !this.internals.validity.valid;
    // this.closeDropdown(); // FIXME should be here to close on component quit using Tab
  }

  private onInput(value?: string): void {
    debounce(() => {
      this.filterResults(value || this.inputElement?.value || '');
  ...
  private onSearchClick(): void {
    ...
    this.searchElement?.classList.add('ods-combobox__search--focused');
  ...
  private onTagRemove(value: string): void {
    ...
    if (this.currentSelections.length === 0 || this.currentTagFocusedIndex >= this.currentSelections.length) {
  ...
  render(): FunctionalComponent {
    // TODO? add onKeyUp={ (event: KeyboardEvent): void => submitFormOnEnter(event, this.internals.form) }
    return (
      <Host
