...
describe('spec:osds-pagination', () => {
  ...
  describe('attributes', () => {
    ...
    describe('totalPages 1->100 with odsUnitTestAttribute', () => {
      odsUnitTestAttribute<OdsPaginationAttributes, 'totalPages'>({
        ...getAttributeContextOptions<OdsPaginationAttributes, OsdsPagination, 'totalPages'>({
          name: 'totalPages',
          list: [...Array(100).keys()].map(i => i + 1),
          defaultValue: odsPaginationDefaultAttributes.totalPages,
          ...config,
        }),
      });
    });
  ...
  it('handleNextKeyDown', async () => {
    ...
    const event = new KeyboardEvent('keyDown', { code: 'Space', keyCode: 32, bubbles: true });
    const pageList = instance.controller.createPageList(instance.totalPages, instance.current);
  ...
  it('handleNextKeyDown controller', async () => {
    ...
    instance.handleNextKeyDown(new KeyboardEvent('keydown', { key: 'Enter' }), instance.current, instance.controller.createPageList(instance.totalPages, instance.current));
