...
import { IdentityCard2 } from '../../../src/components/identityCard/IdentityCard';
...
<IdentityCard2 aliases={ ['Collapsible', 'FAQ'] }
...
2. **Icon**: A "down/up" chevron <StorybookLink kind="ODS Components/Icon" label="Icon" story="Documentation" /> is used to indicate the “expand/collapse” action, though the entire header area is clickable (always visible).
...
<BestPractices
  ...
  dos={[
    <span>- Header <StorybookLink kind="ODS Components/Text" label="Text" story="Documentation" /> describes the content contained in the panel. Use a clear and concise <StorybookLink kind="ODS Components/Text" label="Text" story="Documentation" />, and be explicit about the action that will happen if the option is selected</span>,
    <span>- Use sentence-style capitalization in header <StorybookLink kind="ODS Components/Text" label="Text" story="Documentation" /> (only the first word in a phrase and any proper nouns capitalized), no more than three words, and no punctuation</span>,
    <span>- Use Accordion instead of <StorybookLink kind="ODS Components/Tabs" label="Tab" story="Documentation" /> if users need to see multiple sections at once</span>,
  ]}
/>
...
The header part can be navigated through tabulation, with the same behavior as a <StorybookLink kind="ODS Components/Button" label="Button" story="Documentation" />.
