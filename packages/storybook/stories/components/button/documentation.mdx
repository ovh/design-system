...
<Icon aria-hidden="true"
      ...
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce "My button".

<Heading label="Icon Button" level={ 3 } />
If a Button contains only an Icon, it must include an aria-label to provide context.

<Canvas of={ ButtonStories.AccessibilityExplicitLabel } sourceState="shown" />
<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce "Clear".
...
<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> When the Button is focused, the screen reader should announce "Button, Filter your search".
