...
<div>
  <div id='description'>
    **Buttons** are trigger-able elements that are used to set actions. They communicate calls to action to the user and
...
**Buttons** are mainly used for actions, like adding, removing, validation, etc.
They are used to permit users to interact with the page or its content.
...
**Buttons**don't usually redirect on an external page. For that, see [**Link**](?path=/docs/ods-components-actions-link--documentation).
