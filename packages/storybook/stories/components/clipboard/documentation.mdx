...
<IdentityCard aliases={ ['Copy Component', 'Copy to Clipboard'] }
              ...
              githubUrl="https://github.com/ovh/design-system/tree/master/packages/ods-react/src/components/clipboard"
              name="Clipboard">
...
<Anatomy src="components/clipboard/anatomy.png" />
...
Pressing <Kbd>Ctrl</Kbd> + <Kbd>C</Kbd> (or <Kbd>Cmd</Kbd> + <Kbd>C</Kbd> on macOS) while the input field is focused copies the selected text.

Pressing <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> when the copy button is focused triggers the copy action (button component behavior).
...
<Canvas of={ ClipboardStories.AccessibilityFormField } sourceState="shown" />
