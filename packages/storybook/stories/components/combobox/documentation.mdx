...
1. <StorybookLink kind={ ODS_COMPONENTS_TITLE.input } label="Input" story={ STORY.documentation } /> where the user types the search query. It displays the current input value or selected tags (multiple selection mode)
2. **Dropdown list** displaying a scrollable list of suggested items. Items can be customized using a custom renderer.
3. <StorybookLink kind={ ODS_COMPONENTS_TITLE.tag } label="Tag" story={ STORY.documentation } /> _(multiple selection mode)_ to display selected items as tags inside the Input field
4. **Clearable** <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } label="Button" story={ STORY.documentation } /> _(optional)_ to allow users to clear the input content
5. <StorybookLink kind={ ODS_COMPONENTS_TITLE.spinner } label="Spinner" story={ STORY.documentation } /> _(optional)_ to indicate that data is being fetched
...
<BestPractices
  donts={[
    ...
    '- Avoid using Combobox when the number of items is very small, you can use Select or Radio',
  ]}
  dos={[
...
If the user exists the field without selecting an item, the input reverts to the placeholder or the last selected value (if any).

Users cannot select the same item twice; already selected items are removed from the dropdown.

<Heading label="Multiple selection mode" level={ 4 } />

Clicking on an item add it as a tag inside the Input field.

Selected items do not reappear in the dropdown to avoid duplicates.

Clicking on a tag removes it.
If a previously added entry tag is removed, its corresponding item does not reappear in the dropdown.
...
No looping with `arrow` keys: The navigation stops at the first and last item, it does not loop back.
...
Pressing `Backspace` has the same behavior as in single selection mode.

Pressing `Enter` removes the currently "focused" tag.
