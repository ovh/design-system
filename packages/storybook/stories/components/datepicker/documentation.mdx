...
<BestPractices
  ...
  dos={[
    ...
    '- Add a label alongside the input to clearly indicate the expected format, especially when the placeholder disappears on inpute',
...
When the **Datepicker** icon button can receive the focus. When the dropdown calendar opens, the focus is moved to the currently selected date, or to today’s date if none is selected.
...
- `Arrow Up`: Move to the same weekday in the previous week
- `Arrow Down`: Move to the same weekday in the next week
