...
To ensure proper accessibility, the **Datepicker** input field must be correctly labeled, linked to additional information, provide clear guidance on date formats and provide meaningful context when clearable button is used.
...
<Icon name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} />  Screen readers will announce the label, the editable input and its content.
...
<Icon name={ ICON_NAME.circleCheck }
       style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce the label, the editable input, its content and the helper.
