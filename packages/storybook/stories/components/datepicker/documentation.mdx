...
<IdentityCard aliases={ ['Calendar'] }
              extraLinks={ [<StorybookLink title={ HOME_TITLE.forms }>Form Guidelines</StorybookLink>] }
...
<Canvas of={ DatepickerStories.AccessibilityFormField } sourceState="shown" />
...
<Canvas of={ DatepickerStories.AccessibilityDateFormat } sourceState="shown" />

<Icon name={ ICON_NAME.circleCheck }
       style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce the label, the field, its content and the helper.

<Canvas of={ DatepickerStories.AccessibilityI18n } sourceState="shown" />
