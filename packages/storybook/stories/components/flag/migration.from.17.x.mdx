...
<Meta title="ODS Components/Content/Flag/Migration From 17.x" />
...
This whole config should not be enforced by ODS and should be manage by each the way they prefer.
...
But, flags are high quality svg that add way to much weight to the bundle, thus it is not a viable solution.

### Externalize the assets

Putting all the flags svg to a remote dedicated server/cdn and embed only the url would be, by far, the most
performant solution.

Sadly this is not possible yet due to internal security rules.

We do hope a solution will be found at some point, that will allow us to put back this component.

### Setup the assets path manually
