...
To ensure proper accessibility, the **Input** component must be correctly labeled, linked to additional information, and provide meaningful context when interactive elements (such as icon buttons) are used.

<Heading label="Always provide an explicit label" level={ 3 } />

Every **Input** must have a clear and explicit label to ensure that users (especially screen reader users) understand its purpose, using either **FormField** or native label tag.

<Canvas of={ InputStories.AccessibilityFormField } sourceState="shown" />
<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce: the label, the editable input and the content of the input.
