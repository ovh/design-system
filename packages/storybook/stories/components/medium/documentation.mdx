import { Canvas, Markdown, Meta } from '@storybook/blocks';
import { Banner } from '../../banner';
import * as MediumStories from './medium.stories';
import { OdsIcon } from '@ovhcloud/ods-components/react';
import { ODS_ICON_NAME } from '@ovhcloud/ods-components';

<Meta of={ MediumStories } name='Documentation' />

<Banner of={ MediumStories } />

_**Medium** is a wrapper for displaying assets such as images_

<Canvas of={ MediumStories.Overview } sourceState='none' />

## Overview
<div>
  <div id='description'>

  </div>
  <table>
    <tbody>
    <tr>
      <th scope="row">Name</th>
      <td><strong>Medium</strong></td>
    </tr>
    <tr>
      <th scope="row">Also known as</th>
      <td><em>Media, Image</em></td>
    </tr>
    <tr>
      <th scope="row">Atomic type</th>
      <td>
        <a href='https://atomicdesign.bradfrost.com/chapter-2/#molecules'>Molecule<OdsIcon
          name={ ODS_ICON_NAME.externalLink }
          style={ { fontSize: '.7rem', paddingLeft: '.3rem', paddingRight: '.2rem' } }></OdsIcon></a>
      </td>
    </tr>
    <tr>
      <th scope="row">Related component(s)</th>
      <td>-</td>
    </tr>
    <tr>
      <th scope="row">Links</th>
      <td>
        <a href='https://www.figma.com/proto/tIKzHa5KvHHyosgIgyBswB/Design-Tokens'>Design<OdsIcon
          name={ ODS_ICON_NAME.externalLink }
          style={ { fontSize: '.7rem', paddingLeft: '.3rem', paddingRight: '.2rem' } }></OdsIcon></a><br />
        <a href='https://github.com/ovh/design-system/tree/master/packages/components/src/medium'>Github<OdsIcon
          name={ ODS_ICON_NAME.externalLink }
          style={ { fontSize: '.7rem', paddingLeft: '.3rem', paddingRight: '.2rem' } }></OdsIcon></a>
      </td>
    </tr>
    </tbody>
  </table>
</div>

## Anatomy

![Component anatomy](components/medium/anatomy.png "Component anatomy")

**Medium** only has a single main content.

## Usage

**Medium** is used to display assets with an alternative [**Text**](?path=/docs/ods-components-content-tag--documentation) description.

When the asset is an image, it should be mostly presentation to draw user's attention visually.

### Dos & Don'ts

<Markdown>
  { `
| ✅ Do                                                                                                                                                                           |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - Provide captions or descriptions for media content where applicable.                                                                                                         |
` }
</Markdown>
<Markdown>
  { `
| ❌ Don't                                                                                                                                                                          |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - Avoid assets with text in them since screen readers won't be able to read the words.                                                                                           |
| - Avoid unnecessary decoration                                                                                                                                                   |
` }
</Markdown>

## Placement

The width and height dimensions of the image can be set to fixed sizes.

## Behavior

N/A

## Variation

N/A

## Accessibility

An alternative [**Text**](?path=/docs/ods-components-content-tag--documentation) description is mandatory to describe the information the asset may bring to the user.
