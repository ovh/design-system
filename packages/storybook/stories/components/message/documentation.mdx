...
1. <StorybookLink kind="ODS Components/Icon" label="Icon" story="Documentation" /> to inform users about what kind of message it is (optional)
...
The `close` icon <StorybookLink kind="ODS Components/Button" label="Button" story="Documentation" /> is focusable and can be triggered with the `Enter`/`Space` key.
