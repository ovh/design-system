...
<Anatomy src="components/message/anatomy.png" />
...
Pressing <Kbd>Tab</Kbd> moves focus to the close icon button (if available).

Pressing <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> while the close icon button is focused should trigger the action to dismiss the **Message**.
...
<Canvas of={ MessageStories.AccessibilityAlternativeGrouping } sourceState="shown" />
