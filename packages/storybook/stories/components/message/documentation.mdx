...
<div>
  <div id='description'>
    **Message** component communicate information to the user.
  ...
  <table>
    ...
    <tr>
      ...
      <td><em>Notifier, Notification, Banner, Alert, Feedback</em></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        <a href='https://github.com/ovh/design-system/tree/master/packages/components/src/message'>Github<OdsIcon
          name={ ODS_ICON_NAME.externalLink }
          style={ { fontSize: '.7rem', paddingLeft: '.3rem', paddingRight: '.2rem' } }></OdsIcon></a>
      </td>
    </tr>
...
1. [**Icon**](?path=/docs/ods-components-content-icon--documentation) to inform users about what kind of message it is (optional)

2. [**Text**](?path=/docs/ods-components-content-tag--documentation), which can be a plain text or contain a link

3. A `cross` icon [**Button**](?path=/docs/ods-components-actions-button--documentation), which is a close button to dismiss the **Message**
...
- **Danger**: Reserved for errors, malfunctions, as well as critical issues. Inform the user that a problem requires immediate intervention or correction before the process continues.
...
A click on the `close` icon [**Button**](?path=/docs/ods-components-actions-button--documentation) will dismiss the **Message**.
...
The `close` icon [**Button**](?path=/docs/ods-components-actions-button--documentation) is focusable and can be triggered with the `Enter`/`Space` key.
