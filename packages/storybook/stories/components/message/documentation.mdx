...
To ensure proper accessibility, the Message component should be properly structured.

<Heading label="Best practices" level={ 3 } />

<Heading label="Structuring multiple Messages" level={ 4 } />

When displaying multiple Messages, wrap them in a structured list to improve screen reader announcements.

<Canvas of={ MessageStories.AccessibilityGrouping } sourceState="shown" />
...
Screen readers will announce "List, 2 items" and navigate efficiently.

<Heading label="Setting the correct ARIA role" level={ 4 } />

For non-critical Messages (e.g., info, success, warning), use `role="status"`.

<Canvas of={ MessageStories.AccessibilityRoleStatus } sourceState="shown" />

For critical Messages, use `role="alert"`.

<Canvas of={ MessageStories.AccessibilityRoleAlert } sourceState="shown" />

<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> This ensures that screen readers announce the Message at the appropriate time without interrupting the user.
