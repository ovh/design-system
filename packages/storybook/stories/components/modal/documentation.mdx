...
<div>
  <div id='description'>
    ...
    dismiss or click on an action [**Button**](?path=/docs/ods-components-button--documentation) to proceed
    further.
  </div>
  <table>
    <tbody>
...
2. **Container**
3. **Content** container, filled with [**Text**](?path=/docs/ods-components-text--documentation) or other read-only content, depending on the usage
...
When necessary, depending on the viewport height and the **Modal** content, the user can scroll vertically with the header and the action [**Button**](?path=/docs/ods-components-button--documentation)) remaining in place.
...
- `Tabulation` allows to move forward, first to the closing icon, then through any focusable element in the **Modal** (e.g. action [**Button**](?path=/docs/ods-components-button--documentation)))
