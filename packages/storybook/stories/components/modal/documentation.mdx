...
<IdentityCard aliases={ ['Dialog', 'Alert Dialog', 'Confirm Dialog'] }
              ...
              githubUrl="https://github.com/ovh/design-system/tree/master/packages/ods-react/src/components/modal"
              name="Modal">
...
To ensure the Modal component is fully accessible, correct ARIA attributes must be implemented. This ensures screen readers announce the Modal properly and that keyboard users can interact with it effectively.

<Heading label="Linking the Modal title and content" level={ 3 } />

Ensure that assistive technologies announce the Modal correctly using `aria-labelledby` and `aria-describedby`.

<Canvas of={ ModalStories.AccessibilityAria } sourceState="shown" />

<Icon name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce the different section linked by the aria attributes.
...
<Canvas of={ ModalStories.AccessibilityActions } sourceState="shown" />
