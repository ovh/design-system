...
3. **Clear action**: a `close` icon <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } story={ STORY.documentation }>Button</StorybookLink> so that user can clear the Input field content (optional)
4. **Show/hide action**: an `eye` icon <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } story={ STORY.documentation }>Button</StorybookLink> to toggle between masked or visible Input field content
...
A loading state (spinner) can be displayed inside the field when needed, for example during validation or processing.
