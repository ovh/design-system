...
<IdentityCard aliases={ ['Phone Number Field'] }
              ...
              relatedComponents={[
                { name: 'Flag' },
                { name: 'FormField', subtitle: 'Form elements' },
...
2. **Input field**: displays the country indicator as prefix when users select a country, and they can enter their phone number in this field (from 7 to 12 characters maximum). The <StorybookLink kind={ ODS_COMPONENTS_TITLE.input } story={ STORY.documentation }>Input</StorybookLink> field also has a placeholder to provide hints regarding format according to the selected country.
