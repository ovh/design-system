...
1. **Country indicator Select**: displays the selected country flag and the list of countries by click. List is scroll-able, with 5 items visible at the same time. Each country name is written in the user locale, in alphabetical order (for Latin script)
2. **Input field**: displays the country indicator as prefix when users select a country, and they can enter their phone number in this field (from 7 to 12 characters maximum). The [**Input**](?path=/docs/ods-components-form-input--documentation) field has also a placeholder to provide hints regarding format according to the selected country.
...
- As in customer contact/appointment form
...
## Behavior
[
// (By default, the country **Select** displays user's locale country. If the locale does not exist in the Phone Number current selection, the first item is selected by default.)
]  #

Country indicator [**Select**](?path=/docs/ods-components-form-select--documentation) can be optional.
...
Keyboard navigation is the same as [**Select**](?path=/docs/ods-components-form-select--documentation) and [**Input**](?path=/docs/ods-components-form-input--documentation) does, `Tab` key permits to focus to one another.
