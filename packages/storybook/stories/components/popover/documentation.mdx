...
1. **Container**: main container that has all content (e.g., text, Button, etc.)
...
It can include interactive elements such as Button or Link.
...
<BestPractices
  ...
  dos={[
    ...
    '- Prefer Popover over Tooltip when the content is longer, interactive, or requires formatting (but keep it focused)',
    '- Write Popover content using complete sentences',
    '- Ensure the Popover is positioned clearly and does not overlap with its trigger element',
    '- Use Popover for lightweight content such as field explanations, secondary actions',
  ]}
/>
