  ...
  <table>
    ...
    <tr>
      ...
      <td>
        ...
        [**Flag**](?path=/story/ods-components-content-flag--demo),
        [**Badge**](?path=/docs/ods-components-content-badge--documentation), [**Form
    ...
    <tr>
      ...
      <td>
        ...
        <a href='https://github.com/ovh/design-system/tree/master/packages/components/src/select'>Github<OdsIcon
          name={ ODS_ICON_NAME.externalLink }
          style={ { fontSize: '.7rem', paddingLeft: '.3rem', paddingRight: '.2rem' } }></OdsIcon></a>
      </td>
    </tr>
...
3. "Chevron" [**Icon**](?path=/docs/ods-components-content-icon--documentation) to inform that the **Select** can display a list of choices.
...
| - Use only one word or a very short [**Text**](?path=/docs/ods-components-content-tag--documentation) for each item of the list: try to keep it to one line.                   |
` }
</Markdown>
<Markdown>
  { `
| ❌ Don't                                                                                                                                                                          |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - Use a **Select** to display a "yes" or "no" selection: use [**Radio**](?path=/docs/ods-components-form-radio--documentation) buttons                                           |
| - Use a **Select** when a comparison between all items or options is needed: use [**Radio**](?path=/docs/ods-components-form-radio--documentation) buttons                       |
` }
</Markdown>
...
When an item contains a [**Text**](?path=/docs/ods-components-content-tag--documentation) that cannot fit on a single line, the [**Text**](?path=/docs/ods-components-content-tag--documentation) will be truncated.

An event is triggered when the **Select** option changes.

An event is called on focusing, and a blur event is triggered when the **Select** loses the focus.
