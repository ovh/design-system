...
1. **Header** : Header cells of each column. Each header cell contains a text, and can contain an additional icon Button helper with a Tooltip.
2. **Body** : Main content for data visualization. Each cell can contain text and diverse components, such as Badge, Icon, Link, Tooltip, Medium etc.
...
<BestPractices
  donts={[
    ...
    '- Don’t use the Table component to display large datasets (e.g. 30+ rows) without pagination or `Load more` Button',
