...
<IdentityCard aliases={ [] }
              ...
              githubUrl="https://github.com/ovh/design-system/tree/master/packages/ods-react/src/components/table"
              name="Table">
</IdentityCard>
...
* Defining table headers with `<th>` and `scope` attributes
* Using `<caption>` to provide context
* Associating data with headers for screen readers
* Implementing complex tables with aria-labelledby if needed
* And so much more...

By following these recommendations, the **Table** component will be usable for all users, including those relying on assistive technologies.
