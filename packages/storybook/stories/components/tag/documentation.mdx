...
<IdentityCard aliases={ ['Chip (previous name)', 'Label', 'Pill'] }
              ...
              githubUrl="https://github.com/ovh/design-system/tree/master/packages/ods-react/src/components/tag"
              name="Tag">
...
Unlike <StorybookLink kind={ REACT_COMPONENTS_TITLE.badge } story={ STORY.documentation }>Badge</StorybookLink>, **Tags** are interactive and can be removed by the user.
...
When multiple **Tags** are displayed together (e.g., a collection of selected filters or labels), they should be wrapped in a an unordered list of items (`<ul>` and `<li>`) to ensure proper announcement by screen readers.

<Canvas of={ TagStories.inList } sourceState='shown' />

<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> This allows screen readers to announce "List, 3 items" making it clear that these **Tags** belong to a structured set.

<Heading label="Alternative approach" level={ 3 } />

When modifying the HTML structure is not possible, `role="list"` and `role="listitem"` can be used to provide equivalent semantics.

<Canvas of={ TagStories.withAriaRoles } sourceState='shown' />
...
<Canvas of={ TagStories.withAriaLabel } sourceState='shown' />

<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> This ensures that users know the **Tag** can be removed. Screen readers will announce "Remove my tag, button".
