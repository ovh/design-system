...
To ensure proper accessibility, it must be correctly labeled, linked to additional information, and provide live updates when character limits are in place.
...
<Icon aria-hidden="true"
      ...
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce the label as well as the textarea field.

<Heading label="Linking helper text" level={ 3 } />

Every **Textarea** should ensure that helper text is properly announced by screen readers, using
...
<Icon aria-hidden="true"
      name={ ICON_NAME.circleCheck }
      style={{ color: 'var(--ods-color-success-500)' }} /> Screen readers will announce the label, the textarea field and the helper text.
