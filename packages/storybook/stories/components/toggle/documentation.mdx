...
**Toggle** component should be properly labeled and fully operable using screen readers.

<Heading label="Always provide an explicit label" level={ 3 } />

To ensure clarity, the **Toggle** must have an associated label that explains its function.

Using the Form Field component:

<Canvas of={ ToggleStories.AccessibilityFormFieldLabel } sourceState="shown" />
...
Using an aria-label for context:
