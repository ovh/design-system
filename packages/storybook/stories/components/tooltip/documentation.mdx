...
They are suitable for Button, Icon, Link, and any other elements where additional context would be beneficial.

<Heading label="Tooltip vs Popover" level={ 3 } />

- Both components look similar but a **Tooltip** is displayed on hover and focus while a <StorybookLink kind={ ODS_COMPONENTS_TITLE.popover } story={ STORY.documentation }>Popover</StorybookLink> is triggered by click.
