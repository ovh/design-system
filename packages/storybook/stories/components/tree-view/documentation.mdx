...
<Anatomy src="components/tree-view/anatomy.png" />
...
Pressing <Kbd>Arrow Up</Kbd> / <Kbd>Arrow Down</Kbd> moves focus to the previous or next visible node.

Pressing <Kbd>Arrow Right</Kbd>:

- If the focused node is collapsed, expands it
- If the focused node is expanded, moves focus to its first child

Pressing <Kbd>Arrow Left</Kbd>:

- If the focused node is expanded, collapses it
- If the focused node is collapsed, moves focus to its parent

Pressing <Kbd>Home</Kbd>, or <Kbd>fn</Kbd> + <Kbd>Arrow Left</Kbd>, moves focus to the first visible node.

Pressing <Kbd>End</Kbd>, or <Kbd>fn</Kbd> + <Kbd>Arrow Right</Kbd> moves focus to the last visible node.

Pressing <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> activates, expands, or collapses the focused node. If the node has a checkbox, it toggles its state.

Pressing <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> + <Kbd>Shift</Kbd> + <Kbd>Arrow Up</Kbd> / <Kbd>Arrow Down</Kbd> selects a continuous range of nodes (multi-selection mode).

Pressing <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> + <Kbd>Ctrl</Kbd> or <Kbd>Cmd</Kbd> toggles selection of individual node.

Pressing <Kbd>Ctrl</Kbd> or <Kbd>Cmd</Kbd> + <Kbd>A</Kbd> selects all nodes in the tree. When all nodes are already selected, it unselects all nodes.
