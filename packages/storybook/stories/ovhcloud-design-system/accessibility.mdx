...
<Meta title="OVHcloud Design System/Accessibility" />
...
While our components are built with accessibility in mind, using semantic HTML elements by default, and including the necessary ARIA attributes where needed, certain ARIA attributes, roles, and behaviors must be implemented at the integration level to ensure full accessibility in different use cases.

This means that in most cases, you don’t need to add extra ARIA attributes.
...
<Heading label="Avoid overriding auto-accessible elements with redundant ARIA attributes" level={ 4 } />

**OVHcloud Design System** mainly uses semantic HTML elements by default that inherently support accessibility.
Elements such as `<button>`, `<input>` have built-in roles, keyboard support, and screen reader behaviors, so they typically don't require additional ARIA attributes.
...
<Icon aria-hidden="true"
      ...
      style={{ color: 'var(--ods-color-primary-500)' }} /> Use <ExternalLink href="https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-label">aria-label</ExternalLink> when there is no visible label, and <ExternalLink href="https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-labelledby">aria-labelledby</ExternalLink> when there is one.
...
<Icon aria-hidden="true"
      name={ ICON_NAME.circleInfo }
      style={{ color: 'var(--ods-color-primary-500)' }} /> Depending on the screen reader, some additional information might be read aloud, which is confusing and unhelpful.
...
<Icon aria-hidden="true"
      name={ ICON_NAME.circleInfo }
      style={{ color: 'var(--ods-color-primary-500)' }} /> It also matters since it keeps focus on the button, avoiding disruption for keyboard users.
...
<Icon aria-hidden="true"
      name={ ICON_NAME.circleInfo }
      style={{ color: 'var(--ods-color-primary-500)' }} /> Please note you should navigate in a radio group using `arrow` keys.
