...
<OdsIcon name="xmark" style={{ color: 'var(--ods-color-critical-500)' }} /> The following will cause unexpected behaviour as the `value` will get updated twice, back to its original state,
...
<OdsIcon name="check" style={{ color: 'var(--ods-color-success-500)' }} /> To manage this double usage, it's best to separate your local state from the component and update it only on `odsChange` event, like:
