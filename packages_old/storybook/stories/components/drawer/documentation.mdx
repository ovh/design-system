...
The component is a simple container that wraps its whole content.
...
<BestPractices
  ...
  donts={[
    '- Use the Drawer for displaying purely informational content — prefer a Modal or Message component instead',
...
Be careful the placement of the drawer can be broken if you create a new <OdsLink href="https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_context" label="stacking context" target="_blank" />.
...
- `Tabulation` allows to move forward through any focusable element in the **Drawer** (e.g. action <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } label="Button" story={ STORY.documentation } />)
