...
A **Toggle** should never require users to press a <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } label="Button" story={ STORY.documentation } /> to apply their settings (if a setting requires a <StorybookLink kind={ ODS_COMPONENTS_TITLE.button } label="Button" story={ STORY.documentation } />, use a <StorybookLink kind={ ODS_COMPONENTS_TITLE.checkbox } label="Checkbox" story={ STORY.documentation } /> instead).

<Heading label="Dos & Don'ts" level={ 3 } />

<BestPractices
  donts={[
    <span>- Use a Toggle when you are not expecting an immediate effect by clicking on it: use a <StorybookLink kind={ ODS_COMPONENTS_TITLE.checkbox } label="Checkbox" story={ STORY.documentation } /> component instead</span>,
  ]}
  dos={[
